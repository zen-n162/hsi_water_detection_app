...
def _format_status_path(path_value: str | None, *, safe: bool) -> str | None:
    if not path_value:
        return None
    return to_safe_path_label(path_value) if safe else path_value


def build_deploy_asset_status(
    ...
    *,
    ...
    for key in _STATUS_PATH_KEYS:
        path_value = resolved_config.get(key)
        if not path_value:
            continue
        path = Path(path_value)
        assets[key] = {
            "configured": True,
            "exists": path.exists(),
            "path": _format_status_path(str(path), safe=safe_paths),
        }

    model_ready = False
    if "model_checkpoint" in assets:
        model_ready = assets["model_checkpoint"]["exists"]
    elif "spat_checkpoint" in assets or "spec_checkpoint" in assets:
        model_ready = assets.get("spat_checkpoint", {}).get("exists", False) and assets.get(
            "spec_checkpoint", {}
        ).get("exists", False)

    calibration_ready = True
    if "temperature_json" in assets:
        calibration_ready = assets["temperature_json"]["exists"]

    supporting_assets_ready = all(
        assets[key]["exists"]
        for key in ("manifest_path", "dataset_path")
        if key in assets
    )
    missing_assets = [key for key, meta in assets.items() if meta.get("configured") and not meta.get("exists")]

    return {
        "ready_for_inference": model_ready and calibration_ready and supporting_assets_ready,
        "model_ready": model_ready,
        "calibration_ready": calibration_ready,
        "supporting_assets_ready": supporting_assets_ready,
...
def make_deploy_config_response(deploy_config_path: str | None = None) -> dict[str, Any]:
    ...
    asset_status = build_deploy_asset_status(
        resolved,
        safe_paths=settings.is_external_web_mode,
    )
